...
def extract_news(content: str) -> typing.List[typing.Dict[str, typing.Any]]:
    ...
    for i in tree.xpath("//table[@class='itemlist']/tr"):
        if i.get('class') == 'morespace':
            ...
            news.append({
                'url': i.xpath('./td/a')[0].get('href'),
                'title': i.xpath('./td/a')[0].text
            })
        elif i.get('class') == "spacer":
            continue
        else:
            news[-1].update({
                'points': int(i.xpath(".//span[@class='score']")[0].text.split()[0]) if i.xpath(
                    ".//span[@class='score']") else 0,
                'author': i.xpath(".//a[@class='hnuser']")[0].text if i.xpath(".//a[@class='hnuser']") else '',
                'comments': int(i.xpath(".//a")[-1].text.split()[0]) if 'comments' in i.xpath(".//a")[-1].text else 0
            })
    return news
