...
def create_table():
    execute_sql_query("""CREATE TABLE IF NOT EXISTS news (id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT,
...
def add_elements(elements: typing.List[typing.Dict[str, typing.Any]]):
    for element in elements:
        found_elems = get_cursor().execute(
            f"""SELECT id FROM news WHERE title='{normalize_str_for_sql(element['title'])}';""").fetchall()
        if found_elems:
            execute_sql_query(f"""UPDATE news SET comments={element['comments']} WHERE id={found_elems[0][0]}""")
            execute_sql_query(f"""UPDATE news SET points={element['points']} WHERE id={found_elems[0][0]}""")
        else:
            execute_sql_query(f"""INSERT INTO news (title, author, url, comments, points) VALUES
