...
def main():
    ...
    argument_parser.add_argument('--max_gen', type=float, default=float('inf'), help='Max generation count')
    argument_parser.add_argument('--source', type=str, default='', help='Path to file with first generation')
    ...
    if args.source != '':
        ...
        life = GameOfLife(size=(args.rows, args.cols), randomize=args.rand, max_generations=args.max_gen)
    console = Console(life, save_path=args.spath, speed=args.speed)
    console.run()
